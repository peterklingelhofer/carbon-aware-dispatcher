...
Note: Returns XML, which we parse manually (no lxml dependency).
"""
...
ENTSOE_EMISSION_FACTORS = {
    ...
    # B10 Hydro Pumped Storage is storage, so it is excluded from the mix
...
# PSR codes that represent storage
...
def get_forecast(zone, max_carbon, entsoe_token):
    ...
    # Without a baseline VRE share we cannot scale, so bail to "unknown"
