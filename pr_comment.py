...
def build_comment(
    *,
    ...
    if tier and tier != "unknown":
        emoji = {"green": "🟢", "amber": "🟡", "red": "🔴"}.get(tier, "")
        lines.append(f"| Carbon tier | {emoji} {tier} |")
    ...
    if budget:
        emoji = {"ok": "🟢", "warning": "🟡", "exceeded": "🔴"}.get(budget.get("state"), "")
        lines.append(
            f"| Carbon budget | {emoji} {budget.get('used_pct', 0):.0f}% used"
            f" ({budget.get('state', '')}) |"
        )
    lines.append("")
