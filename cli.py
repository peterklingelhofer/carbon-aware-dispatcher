...
Info logs go to stderr, and stdout carries only the result (or JSON with --json), so
...
def _measure_all(args):
    """Measure every candidate zone once and return [(zone, intensity)] (engine logs to stderr)."""
    zones = check_grid.parse_zones_input(args.zones)
    tok = _tokens(args)
    measured = []
    with contextlib.redirect_stdout(sys.stderr):
...
def cmd_advise(args):
    ...
    if not worth:
        print("  Grid is fairly flat here, so time-shifting saves little. Focus on running less:")
    if not actions:
